...
pub struct SimulationFailure {
    ...
    error: rtd_rpc::proto::rtd::rpc::v2::ExecutionError,
}

#[cfg(feature = "intents")]
impl SimulationFailure {
    pub(crate) fn new(error: rtd_rpc::proto::rtd::rpc::v2::ExecutionError) -> Self {
        Self { error }
    }

    /// Returns the underlying execution error.
    pub fn execution_error(&self) -> &rtd_rpc::proto::rtd::rpc::v2::ExecutionError {
        &self.error
    }
    ...
    pub fn kind(
        &self,
    ) -> Option<rtd_rpc::proto::rtd::rpc::v2::execution_error::ExecutionErrorKind> {
        self.error.kind.and_then(|k| {
            rtd_rpc::proto::rtd::rpc::v2::execution_error::ExecutionErrorKind::try_from(k).ok()
        })
    }

    /// Returns the Move abort details, if this was a `MoveAbort` error.
    pub fn move_abort(&self) -> Option<&rtd_rpc::proto::rtd::rpc::v2::MoveAbort> {
    ...
    pub fn clever_error(&self) -> Option<&rtd_rpc::proto::rtd::rpc::v2::CleverError> {
...
impl std::fmt::Display for SimulationFailure {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if let Some(abort) = self.move_abort() {
            ...
            {
                write!(f, " [{name}")?;
                if let Some(rtd_rpc::proto::rtd::rpc::v2::clever_error::Value::Rendered(v)) =
                    &clever.value
                {
                    write!(f, " = {v}")?;
                }
