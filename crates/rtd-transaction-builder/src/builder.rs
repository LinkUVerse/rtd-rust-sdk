...
///
/// [ptb]: https://docs.rtd.io/concepts/transactions/prog-txn-blocks
///
/// # Example
///
/// ```
/// use rtd_sdk_types::Address;
/// use rtd_sdk_types::Digest;
/// use rtd_transaction_builder::ObjectInput;
...
pub(crate) enum ResolvedArgument {
    ...
    Resolved(rtd_sdk_types::Argument),
}
...
pub(crate) enum InputArg {
    ...
    Pure(Vec<u8>),
    Object(ObjectInput),
    FundsWithdrawal(rtd_sdk_types::FundsWithdrawal),
}
...
impl TransactionBuilder {
    ...
    pub fn funds_withdrawal(&mut self, coin_type: TypeTag, amount: u64) -> Argument {
        let funds_withdrawal = rtd_sdk_types::FundsWithdrawal::new(
            amount,
            coin_type,
    ...
    ///
    /// This adds a [`FundsWithdrawal`](rtd_sdk_types::FundsWithdrawal) input and calls
    ...
    ///     )
    ///     .with_type_args(vec!["0x2::rtd::RTD".parse().unwrap()]),
    ///     vec![],
    /// );
    /// ```
    ...
    ///
    /// let mut tx = TransactionBuilder::new();
    /// let coin = tx.intent(Coin::rtd(1_000_000_000));
    ...
    ///
    /// let mut tx = TransactionBuilder::new();
    /// let coin = tx.coin(StructTag::rtd(), 1_000_000_000);
    /// ```
    #[cfg(feature = "intents")]
    #[cfg_attr(doc_cfg, doc(cfg(feature = "intents")))]
    pub fn coin(&mut self, coin_type: rtd_sdk_types::StructTag, amount: u64) -> Argument {
    ...
    ///
    /// let mut tx = TransactionBuilder::new();
    /// let bal = tx.balance(StructTag::rtd(), 1_000_000_000);
    /// ```
    #[cfg(feature = "intents")]
    #[cfg_attr(doc_cfg, doc(cfg(feature = "intents")))]
    pub fn balance(&mut self, coin_type: rtd_sdk_types::StructTag, amount: u64) -> Argument {
    ...
    pub fn try_build(mut self) -> Result<Transaction, Error> {
        ...
        // Gas payment
        let gas_payment = rtd_sdk_types::GasPayment {
            objects: self
                .gas
                .iter()
        ...
        for (id, input) in unresolved_inputs {
            let arg = match input {
                InputArg::Gas => rtd_sdk_types::Argument::Gas,
                InputArg::Pure(value) => {
                    resolved_inputs.push(rtd_sdk_types::Input::Pure(value));
                    rtd_sdk_types::Argument::Input(resolved_inputs.len() as u16 - 1)
                }
                ...
                InputArg::FundsWithdrawal(funds_withdrawal) => {
                    resolved_inputs.push(rtd_sdk_types::Input::FundsWithdrawal(funds_withdrawal));
                    rtd_sdk_types::Argument::Input(resolved_inputs.len() as u16 - 1)
                }
            };
        ...
        for (id, command) in self.commands {
            ...
            let arg = rtd_sdk_types::Argument::Result(resolved_commands.len() as u16 - 1);

            *self.arguments.get_mut(&id).unwrap() = ResolvedArgument::Resolved(arg);
        }

        Ok(Transaction {
            kind: rtd_sdk_types::TransactionKind::ProgrammableTransaction(
                rtd_sdk_types::ProgrammableTransaction {
    ...
    #[cfg(feature = "intents")]
    #[cfg_attr(doc_cfg, doc(cfg(feature = "intents")))]
    pub async fn build(mut self, client: &mut rtd_rpc::Client) -> Result<Transaction, Error> {
        ...
        for (id, input) in unresolved_inputs {
            let arg = match input {
                ...
                InputArg::FundsWithdrawal(funds_withdrawal) => {
                    resolved_inputs
                        .push(rtd_sdk_types::Input::FundsWithdrawal(funds_withdrawal).into());
                    rtd_sdk_types::Argument::Input(resolved_inputs.len() as u16 - 1)
                }
            };
...
impl Argument {
    ...
    ) -> Result<rtd_sdk_types::Argument, Result<usize, Error>> {
    ...
    fn try_resolve_many(
        arguments: &[Self],
        resolved_arguments: &BTreeMap<usize, ResolvedArgument>,
    ) -> Result<Vec<rtd_sdk_types::Argument>, Result<usize, Error>> {
        arguments
            .iter()
...
impl Command {
    fn try_resolve(
        &self,
        resolved_arguments: &BTreeMap<usize, ResolvedArgument>,
    ) -> Result<rtd_sdk_types::Command, Result<usize, Error>> {
        use rtd_sdk_types::Command as C;
        ...
        let cmd = match &self.kind {
            CommandKind::MoveCall(MoveCall {
                ...
                type_arguments,
                arguments,
            }) => C::MoveCall(rtd_sdk_types::MoveCall {
                package: *package,
            ...
            CommandKind::TransferObjects(TransferObjects { objects, address }) => {
                C::TransferObjects(rtd_sdk_types::TransferObjects {
            ...
            CommandKind::SplitCoins(SplitCoins { coin, amounts }) => {
                C::SplitCoins(rtd_sdk_types::SplitCoins {
            ...
            CommandKind::MergeCoins(MergeCoins {
                coin,
                coins_to_merge,
            }) => C::MergeCoins(rtd_sdk_types::MergeCoins {
                coin: coin.try_resolve(resolved_arguments)?,
            ...
            CommandKind::Upgrade(Upgrade {
                ...
                package,
                ticket,
            }) => C::Upgrade(rtd_sdk_types::Upgrade {
                modules: modules.to_owned(),
                dependencies: dependencies.to_owned(),
                package: *package,
...
impl From<&rtd_sdk_types::Object> for ObjectInput {
    fn from(object: &rtd_sdk_types::Object) -> Self {
        let input = Self::new(object.object_id())
            .with_version(object.version())
            .with_digest(object.digest());

        match object.owner() {
            rtd_sdk_types::Owner::Address(_) => input.as_owned(),
            rtd_sdk_types::Owner::Object(_) => input,
            rtd_sdk_types::Owner::Shared(version) => input.with_version(*version).as_shared(),
            rtd_sdk_types::Owner::Immutable => input.as_immutable(),
            rtd_sdk_types::Owner::ConsensusAddress { start_version, .. } => {
...
impl ObjectInput {
    ...
    fn try_into_input(&self) -> Result<rtd_sdk_types::Input, Error> {
        let input = match self {
            ...
            | Self {
                ...
                digest: Some(digest),
                mutable: None,
            } => rtd_sdk_types::Input::ImmutableOrOwned(rtd_sdk_types::ObjectReference::new(
            ...
            Self {
                ...
                digest: Some(digest),
                ..
            } => rtd_sdk_types::Input::Receiving(rtd_sdk_types::ObjectReference::new(
                *object_id, *version, *digest,
            )),
            ...
            | Self {
                ...
                digest: None,
                mutable: Some(mutable),
            } => rtd_sdk_types::Input::Shared(rtd_sdk_types::SharedInput::new(
    ...
    #[cfg(feature = "intents")]
    fn to_input_proto(&self) -> rtd_rpc::proto::rtd::rpc::v2::Input {
        use rtd_rpc::proto::rtd::rpc::v2::input::InputKind;

        let mut input =
            rtd_rpc::proto::rtd::rpc::v2::Input::default().with_object_id(self.object_id);
        match &self.kind {
    ...
    fn try_into_object_reference_proto(
        &self,
    ) -> Result<rtd_rpc::proto::rtd::rpc::v2::ObjectReference, Error> {
        if !matches!(self.kind, Some(ObjectKind::ImmutableOrOwned) | None) {
            return Err(Error::WrongGasObject);
        }

        let mut input =
            rtd_rpc::proto::rtd::rpc::v2::ObjectReference::default().with_object_id(self.object_id);
        if let Some(version) = self.version {
            input.set_version(version);
        }
        if let Some(digest) = self.digest {
    ...
    pub(crate) fn try_from_object_proto(
        object: &rtd_rpc::proto::rtd::rpc::v2::Object,
    ) -> Result<Self, Error> {
