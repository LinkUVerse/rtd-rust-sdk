...
//!
//! Rtd accounts hold funds in two forms: **coin objects** (`Coin<T>`)
...
///
/// // Request 1 RTD (uses gas coin by default)
/// let coin = Coin::rtd(1_000_000_000);
///
...
impl Coin {
    ...
    /// use rtd_transaction_builder::intent::Coin;
    ///
    /// let coin = Coin::new(StructTag::rtd(), 1_000_000_000);
    /// ```
    ...
    /// Shorthand for requesting a RTD coin with the given balance.
    ///
    ...
    /// use rtd_transaction_builder::intent::Coin;
    ///
    /// let one_rtd = Coin::rtd(1_000_000_000);
    /// ```
    pub fn rtd(balance: u64) -> Self {
        Self {
            coin_type: StructTag::rtd(),
            balance,
            use_gas_coin: true,
        }
    }
    ...
    ///
    /// This is only meaningful when the coin type is RTD. Pass `false` to force the resolver
    /// to select non-gas RTD coins instead.
    ///
    /// ```
    /// use rtd_transaction_builder::intent::Coin;
    ///
    /// // Don't touch the gas coin -- select other RTD coins instead
    /// let coin = Coin::rtd(1_000_000_000).with_use_gas_coin(false);
    /// ```
...
///
/// // Request 1 RTD as a Balance<RTD>
/// let bal = Balance::rtd(1_000_000_000);
///
...
impl Balance {
    ...
    /// use rtd_transaction_builder::intent::Balance;
    ///
    /// let bal = Balance::new(StructTag::rtd(), 1_000_000_000);
    /// ```
    pub fn new(coin_type: StructTag, balance: u64) -> Self {
        Self {
            coin_type,
    ...
    /// Shorthand for requesting a RTD balance with the given amount.
    ///
    ...
    /// use rtd_transaction_builder::intent::Balance;
    ///
    /// let one_rtd = Balance::rtd(1_000_000_000);
    /// ```
    pub fn rtd(balance: u64) -> Self {
        Self {
            coin_type: StructTag::rtd(),
    ...
    /// use rtd_transaction_builder::intent::Balance;
    ///
    /// let bal = Balance::rtd(1_000_000_000).with_use_gas_coin(false);
    /// ```
    pub fn with_use_gas_coin(self, use_gas_coin: bool) -> Self {
        Self {
            use_gas_coin,
        ...
        for (id, intent) in builder.intents.extract_if(.., |_id, intent| {
            ...
            if let Some(coin) = intent.downcast_ref::<Coin>() {
                if coin.balance == 0 {
                    ...
                    let coin_type = if coin.coin_type == StructTag::rtd() && coin.use_gas_coin {
                        CoinType::Gas
                    } else {
                ...
                if bal.balance == 0 {
                    ...
                    let coin_type = if bal.coin_type == StructTag::rtd() && bal.use_gas_coin {
                        CoinType::Gas
                    } else {
...
impl Resolver {
    ...
    ///   RTD buffer for gas fees. No remainder handling.
    ...
    async fn resolve_merge_and_split(
        ...
        let (coin_type, use_gas_coin) = match coin_type {
            CoinType::Gas => (StructTag::rtd(), true),
        ...
        let _chain_id = {
            use rtd_rpc::client::ResponseExt;
    ...
    fn prepare_gas_coin_sources(
        ...
        // sender: Address,
        address_balance: u64,
        // chain_info: (Option<rtd_sdk_types::Digest>, Option<u64>),
    ) -> (Argument, Vec<Argument>) {
        let gas = builder.gas();
        let mut deps = Vec::new();
        ...
        // {
        //     let reservation = rtd_sdk_types::ObjectReference::coin_reservation(
        //         coin_type,
        //         reservation_amount,
        //         epoch,
        //         chain_id,
