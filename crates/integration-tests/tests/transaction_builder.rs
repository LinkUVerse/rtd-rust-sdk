use anyhow::Result;
use integration_tests::RtdNetworkBuilder;
...
async fn test_move_call() -> Result<()> {
    ...
    let tx = builder.build(&mut rtd.client).await.unwrap();
...
async fn test_publish() {
    let mut rtd = RtdNetworkBuilder::default().build().await.unwrap();
    let private_key = rtd.user_keys.first().unwrap();
    let sender = private_key.public_key().derive_address();
    ...
    let (bytecode, _package_digest) = rtd.build_package(package_dir.as_ref()).unwrap();
...
async fn test_upgrade() -> Result<()> {
    ...
    let package_dir = concat!(env!("CARGO_MANIFEST_DIR"), "/", "packages/test_publish_v2");
    let (bytecode, package_digest) = rtd.build_package(package_dir.as_ref()).unwrap();
