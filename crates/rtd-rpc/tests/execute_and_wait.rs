...
fn test_transaction() -> (proto::Transaction, String) {
    let transaction = rtd_sdk_types::Transaction {
        ...
        sender: rtd_sdk_types::Address::ZERO,
        gas_payment: rtd_sdk_types::GasPayment {
            objects: Vec::new(),
            owner: rtd_sdk_types::Address::ZERO,
            price: 1,
            budget: 1,
        },
        expiration: rtd_sdk_types::TransactionExpiration::None,
    };
...
async fn execution_error_without_commit_surfaces() {
    ...
    match err {
        rtd_rpc::client::ExecuteAndWaitError::RpcError(status) => {
...
async fn missing_transaction_fails_without_rpc() {
    ...
    assert!(matches!(
        err,
        rtd_rpc::client::ExecuteAndWaitError::MissingTransaction
    ));
}
