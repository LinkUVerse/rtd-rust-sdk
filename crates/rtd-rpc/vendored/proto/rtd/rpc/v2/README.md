## Overview
This package defines a number of APIs for interacting with services in the Rtd
...
- `x-rtd-chain-id`: Chain Id of the current chain
- `x-rtd-chain`: Human-readable name of the current chain
- `x-rtd-checkpoint-height`: Current checkpoint height
- `x-rtd-lowest-available-checkpoint`: Lowest available checkpoint for which
...
- `x-rtd-lowest-available-checkpoint-objects`: Lowest available checkpoint for
...
- `x-rtd-epoch`: Current epoch of the chain.
- `x-rtd-timestamp-ms`: Current timestamp of the chain, represented as number
  of milliseconds from the Unix epoch.
- `x-rtd-timestamp`: Current timestamp of the chain, encoded in the
