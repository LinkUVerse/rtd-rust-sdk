...
/// Extension trait used to facilitate retrieval of Rtd specific data from responses
...
impl ResponseExt for http::header::HeaderMap {
    fn chain_id(&self) -> Option<Digest> {
        self.get(crate::headers::X_RTD_CHAIN_ID)
            .map(http::header::HeaderValue::as_bytes)
            .and_then(|s| Digest::from_base58(s).ok())
    }

    fn chain(&self) -> Option<&str> {
        self.get(crate::headers::X_RTD_CHAIN)
            .and_then(|h| h.to_str().ok())
    }

    fn epoch(&self) -> Option<u64> {
        self.get(crate::headers::X_RTD_EPOCH)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok())
    }

    fn checkpoint_height(&self) -> Option<u64> {
        self.get(crate::headers::X_RTD_CHECKPOINT_HEIGHT)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok())
    }

    fn timestamp_ms(&self) -> Option<u64> {
        self.get(crate::headers::X_RTD_TIMESTAMP_MS)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok())
    }

    fn timestamp(&self) -> Option<&str> {
        self.get(crate::headers::X_RTD_TIMESTAMP)
            .and_then(|h| h.to_str().ok())
    }

    fn lowest_available_checkpoint(&self) -> Option<u64> {
        self.get(crate::headers::X_RTD_LOWEST_AVAILABLE_CHECKPOINT)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok())
    }

    fn lowest_available_checkpoint_objects(&self) -> Option<u64> {
        self.get(crate::headers::X_RTD_LOWEST_AVAILABLE_CHECKPOINT_OBJECTS)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok())
    }
}
