...
impl Client {
    ...
    ///
    ///     // Get RTD balance using the helper
    ///     let rtd_balance = client.get_balance(owner, &StructTag::rtd()).await?;
    ///     if let Some(bal) = rtd_balance {
    ///         println!("RTD balance: {}", bal.total_balance);
    ///     }
    ///
    ...
    pub async fn get_balance(
        ...
        const QUERY: &str = graphql_query!(
            "query($owner: RtdAddress!, $coinType: String!) {
...
mod tests {
    ...
    async fn test_get_balance_found() {
        ...
        let result = client.get_balance(owner, &StructTag::rtd()).await;
        assert!(result.is_ok());
        ...
        assert_eq!(balance.coin_type, StructTag::rtd());
    ...
    async fn test_list_balances_multiple() {
        ...
        assert_eq!(bal1.coin_type, StructTag::rtd());
